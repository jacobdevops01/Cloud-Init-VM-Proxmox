...
```
После этого мы переходим в VM Cloud-init и пишем:
...
После этого можем включить наш VM.


☁️ Что такое Cloud-Init и зачем он нужен

Простыми словами

Cloud-Init — это инструмент, который автоматически настраивает виртуальную машину при первом запуске.

Вместо того чтобы:

заходить в ВМ руками
...
👉 Cloud-Init делает это сам за тебя, автоматически.

🧠 Аналогия

Без Cloud-Init:

Ты каждый раз вручную собираешь компьютер с нуля

С Cloud-Init:

Ты включаешь компьютер, и он уже готов к работе

🔧 Что Cloud-Init может делать

При первом запуске ВМ он может:

👤 создать пользователя

🔑 добавить SSH-ключ

🌐 настроить сеть (DHCP / Static IP)
...
📜 выполнить shell-скрипты

И всё это без ручного входа в систему.

🖥 Пример: без Cloud-Init
...
❌ не повторяемо
```
☁️ Пример: с Cloud-Init

Ты делаешь один template, а дальше:
```bash
- Клонировал ВМ
- Указал IP
- Указал SSH ключ
...
🚀 Зачем Cloud-Init в DevOps

Cloud-Init — это база DevOps-подхода:
...
👉 Terraform создаёт ВМ
👉 Cloud-Init настраивает ОС внутри

✅ Итог одной строкой

Cloud-Init нужен, чтобы ВМ была готова к работе сразу после запуска — без ручной настройки.
